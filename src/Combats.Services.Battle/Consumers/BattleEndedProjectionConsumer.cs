...
/// This is the single source of truth for DB updates related to battle termination.
/// 
/// Design principles:
/// - Redis battle state is the authoritative runtime state (used by Game Engine and SignalR).
/// - Postgres Battle table is a read model / history/projection.
/// - BattleEnded event is the canonical termination signal.
/// - All DB updates for "battle ended" scenarios are driven by this projection consumer.
/// </summary>
public class BattleEndedProjectionConsumer : IConsumer<BattleEnded>
{
    ...
    public async Task Consume(ConsumeContext<BattleEnded> context)
    {
        ...
        if (battle == null)
        {
            ...
            // This can happen if the read model is missing data (e.g., battle was never created in DB)
            _logger.LogWarning(
        ...
        if (battle.State == "Ended" && battle.EndedAt != null)
        {
            ...
            // In a production system, you might want to raise an alert here
            _logger.LogWarning(
