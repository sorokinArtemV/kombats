...
public sealed class TurnDeadlineWorker : BackgroundService
{
    ...
    private async Task ProcessDueBattlesAsync(CancellationToken cancellationToken)
    {
        ...
        var turnResolver = scope.ServiceProvider.GetRequiredService<TurnResolverService>();
        ...
        foreach (var battleId in dueBattles)
        {
            try
            {
                ...
                // TurnResolverService will use CAS to ensure only one resolution happens
                var resolved = await turnResolver.ResolveTurnAsync(battleId, turnIndex, cancellationToken);
